/**
 * The "a `.scriptio` arrived" flow: OS double-click, in-app Open…, or the file
 ...
 * ([ScriptioOpenDialog]) sits at the projects layout so it is reachable from the
 * listing and from inside a project alike.
 *
 * Nothing here decides on the user's behalf: it reads the archive, asks
 * `planScriptioOpen` what opening it would mean, and puts that question on
...
export interface PendingScriptioOpen {
    plan: ScriptioOpenPlan;
...
let pending: PendingScriptioOpen | null = null;
const listeners = new Set<() => void>();

const emit = (): void => listeners.forEach((cb) => cb());

export function subscribeScriptioOpen(callback: () => void): () => void {
...
export const getPendingScriptioOpen = (): PendingScriptioOpen | null => pending;

export function dismissScriptioOpen(): void {
    pending = null;
    emit();
}

/** Read, classify, and put the question on screen. */
export async function offerScriptioOpen(
    bytes: ArrayBuffer,
    fileName: string,
    path: string | null,
): Promise<void> {
    pending = { plan: await planScriptioOpen(bytes), bytes, path, fileName };
    emit();
}

/** Read a path from disk (desktop) and offer it. */
export async function offerScriptioOpenFromPath(path: string): Promise<void> {
    const { readFile } = await import("@tauri-apps/plugin-fs");
    const bytes = await readFile(path);
    const buffer = bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength) as ArrayBuffer;
    await offerScriptioOpen(buffer, fileNameOf(path), path);
}

/** The native picker's filter — one place, so every entry point matches. */
export const SCRIPTIO_FILE_FILTER = { name: "Scriptio", extensions: ["scriptio"] };

/**
 * Ask for a `.scriptio` and run it through the open flow.
 *
 ...
 */
export async function pickAndOfferScriptioFile(): Promise<void> {
    const { open } = await import("@tauri-apps/plugin-dialog");
    const path = await open({ multiple: false, directory: false, filters: [SCRIPTIO_FILE_FILTER] });
    if (typeof path === "string") await offerScriptioOpenFromPath(path);
}
...
async function bindIfPossible(projectId: string, path: string | null): Promise<void> {
    ...
    try {
        ...
        console.warn("[Scriptio] Opened the file but could not bind to it:", error);
    }
}

/** Merge the pending archive into an existing local project. */
export async function acceptScriptioMerge(projectId: string): Promise<string> {
    const current = pending;
    if (!current) throw new Error("No file is waiting to be opened");

    const { applyScriptioUpdate } = await import("@src/lib/adapters/scriptio/scriptio-open");
    await applyScriptioUpdate(projectId, current.bytes);
 ...
 */
export async function acceptScriptioAsNewProject(
    fork: boolean,
    ...
    const projectId = await createProjectFromScriptio(current.bytes, {
        fork,

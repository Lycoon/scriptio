/**
 * Drives `PROJECT_MIGRATIONS` against a single `ProjectState`. Called from
 * `useLocalPersistence` after `y-indexeddb` has fully synced and **before**
 * the cloud `WebsocketProvider` connects, to avoid concurrent migrations
 * across clients producing duplicated effects.
 */
...
function readVersion(ydoc: ProjectState): number {
    ...
    return typeof stored === "number" ? stored : LEGACY_PROJECT_VERSION;
}

/**
 * Migrate a project Y.Doc to `CURRENT_PROJECT_VERSION`.
 *
 * Returns an outcome describing what happened. The caller decides UI
 * (block load on `future-version`/`failed`, show the doc on `up-to-date`/`migrated`).
 */
export async function migrateProjectDoc({
    ...
    // Snapshot the doc before mutating so a failed step can be rolled back.
    const snapshot = Y.encodeStateAsUpdate(ydoc);
    const store = backupStore ?? (await defaultBackupStore());
    await store.saveMigrationBackup(projectId, snapshot, stored);

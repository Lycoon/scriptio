...
import { createCharacterHighlightExtension } from "@src/lib/screenplay/extensions/character-highlight-extension";
import { createTestEditor } from "../helpers/editor-factory";
import { pluginBenchSuite } from "../helpers/bench-suite";

const HIGHLIGHTED = new Set(["EDWARD", "WILL", "SANDRA", "JOSEPHINE"]);
    ...
    return createTestEditor(
        [
            ...
            createCharacterHighlightExtension({
                getHighlightedCharacters: () => HIGHLIGHTED,
                getCharacterColor: () => "#6366f1",
            }),
        ],
        content,
    );
...
pluginBenchSuite("character-highlight — apply() timing", makeEditor);
